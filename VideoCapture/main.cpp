...
int main()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        if(codec == 0)
        {
            ...
            videoWrite[i].open(lisen_src_gst_H264_send_video(ip,5000+i),fourcc, (double)30, cv::Size(640, 480), true);
        }
        else if(codec == 1)
        {

            int fourcc = cv::VideoWriter::fourcc('J','P','E','G');
            videoWrite[i].open(lisen_src_gst_JPEG_send_video(ip,5000+i),fourcc, (double)30, cv::Size(640, 480), true);
        }
...
void stream_write(cv::VideoCapture* videoCap, cv::VideoWriter* videoWrite)
{
    ...
    while(videoCap->isOpened())
    {
        ...
        if(videoWrite->isOpened())
        {
            ...
            std::cout << "write frame " << n_frame++ << "\r";
        }
